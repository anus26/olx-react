...
<div className='group  imagewrapper w-full h-[220px] overflow-hidden rounded-md'>
    ...
    {items.map((item)=>(
        ...
        <div key={item.key} className='relative w-full h-full overflow-hidden'>

<img src={`./images/${item.key}.jpg`} alt="images" className='w-full h-full object-cover transition-all duration-500'/>
<img src="./images/w1.jpg" alt="images" className='w-full h-full object-cover left-0 top-0  transition-all duration-500 opacity-0 absolute group-hover:opacity-100'/>
    </div>
    ))}
    </Link>
